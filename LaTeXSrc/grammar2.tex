...
\textbf{定语从句 = 形容词性关系从句（见\cref{chap:adjclause}） + 关系副词从句（见\cref{chap:reladv}）}


\section{限制性和非限制性关系分句}

可根据关系从句与其先行词的关系，将之分为限制性和非限制性两种。
\index{概念!限制性修饰语}\index{概念!非限制性修饰
  语}\index{概念! 限制性关系分句}\index{概念!非限制性关系分句}

\begin{description}
\item[限制性关系分句] 限制性关系分句用来\textbf{限定或明确其修饰的名词或代词}，提
  ...
  \item I need the book \unbf{that you borrowed from the library}.
    ...
    上两句起限制性关系分句作用的从句也被叫做\textbf{限制性修饰语}。

  \end{itemize}


\item[非限制性关系分句] 非限制性关系分句提供的是\textbf{附加的、非关键性的信息}。
...
\item \sout{I won't see \unbf{anyone}, \unbf{who has not made an
  ...
  \textbf{非断定的中心词}如any- 等\textbf{不能有非限制性关系从句，因为：any-
    等非断定代词本就无法限定，再施加非限制性关系从句毫无意义。}像 any,
  all 和 every 等限定词通常只有限制性修饰语。

  \begin{itemize}
  \item \sout{All the students\unbf{, who had failed the test,} wanted to
      try again.} [误，应去修饰语前后的逗号，转为限制性从句，指考试不合格的所
    有学生。]

  \end{itemize}

  偶尔这样的句子里也可以使用限制性修饰语。如下：
  \begin{itemize}
  \item All the students, \unbf{who had returned from their vacation}, wanted
    to take the exam.

    \textbf{所有学生都是度假归来的}，“度假归来”只是附加信息。
  \end{itemize}
...
\textbf{关系代词（见\cref{tab:relativePro}）引导形容词性关系从句，总是放在关
  系从句开头，that或零不能做非限制性关系代词。}
...
当先行词被一个\textbf{最高级形容词或后置限定词(first, last, next, only)}修饰 时，
充当关系从句\textbf{主语}的关系代词通常是 \textbf{that}，充当宾语的关系代词
用 \textbf{that 或零形式}多于用 which 或 who(m)：
\begin{itemize}
...
当代词为 \textbf{which} （使用频率较低，更加正式）时，在下列三个例子中
...
\begin{itemize}
\item James knows \unbf{more about the theater} is more than Susan \unbf{knows}.
\end{itemize}
...
as \ldots{} as 可作：
\begin{description}
\item[限定词] Isabelle has \unbf{as many} books \emph{as} her brother (has).

\item[名词短语的中心词] \unbf{As many} of his friends are in New York \emph{as} (are) here.

\item[下加状语] I agree with you \unbf{as much} \emph{as} ((I agree) with) Robert.

\item[形容词中心语的修饰语] The article was \unbf{as objective} \emph{as} I expected (it
  would be).

\item[被前置形容词修饰的中心语] It was \unbf{as lively a discussion} \emph{as} we thought it would be.

  [形容词短语也可后置 It was a discussion \unbf{as lively} \emph{as} \ldots{}]

\item[副词的修饰语] I am \unbf{as severely} handicapped \emph{as} you (are).

  [副词也可后置 I am handicapped as severely \emph{as} \ldots{}]
\end{description}
...
当母句是否定句时,可以用 so \ldots{} as 取代 as \ldots{} as, \textbf{从句全部
  或大部省略时尤其如此}：
\begin{itemize}
...
\begin{description}
\item[足量比较] enough to足够
  \begin{itemize}
  \item They're rich \unbf{enough to} own a car.

  \item The book is simple \unbf{enough to} understand.
  \end{itemize}


\item[超越比较] \textbf{too有“太过于……”的偏否定的意思。}too + 形容
  词/副词 + to表示“\textbf{太/过于……（以至于）不能}”。
  \begin{itemize}
  \item She is too ill to move. [“她病的太重，以至于不能动”或 “她病重到不能动”。]

  \item They're not \unbf{too poor to} own a car.

    [加not，是对后面too \ldots{} to \ldots{} 的否定，而不是对 not too
    \ldots{}的否定。上句话的中文意思是“他们还没有\textbf{太穷以至于不能买
      车}。”，由此，更进一步和更好的翻译是“\textbf{他们还没有穷到买不起车}”。]

  \item The book is not \unbf{too difficult to} understand.

    [“这本书没有难到不能理解”或“这本书不是太难理解”。]
  \end{itemize}
...
更为正式的结构so/such \ldots{} as+to不定式，有时替代so/such \ldots{} that从句：

\begin{description}
\item[so (形容词/副词) as to] 为了/以便于做某事。
  \begin{itemize}
  \item  We went early \unbf{so as to} get good seats.

  \item I'm not \unbf{so stupid as to} believe that.

  \item Would you be \unbf{so kind as to} lock the door when you leave?
  \end{itemize}

\item[such 名词短语 as to] 如此……以至于
  \begin{itemize}
  \item It was \unbf{such a difficult exam as to} discourage many students from taking it again.

    [这是一次如此难的考试，以致于许多学生都不愿再考了。]

  \item She spoke in \unbf{such a low voice as to} be barely heard in the back row.
...
\item[介词副词] about, above, across, after, along, around, by, down, in,
  off, on, out (AmE),over, past, round, through, under, up,
  etc.\textbf{介词副词}类既可作介词也可作副词。
...
本节不需要多了解，只需要知道\textbf{多词动词在语义上是一个整体}，这常常表现在
它\textbf{可用一个动词来替代}。\textbf{本节内容可以暂且略过，需要时再看}。
...
短语动词和自由组合的差异：
\begin{itemize}
\item 诸如give in（投降）和（blow up）爆炸这样的不及物短语动词,我们无法孤立地
  根据动词和小品词的意思来预测组合后习语的意思。但是在自由组合中（如: walk
  past）,我们就可以做出预测。
...
动词补足关系的分类，其实几乎等同于\textbf{句法成分分析}。其实有多种句法成分划
分方式，并且无论哪一种方式在涉及到复杂句子时都会难度陡增。\textbf{选择一种足
  够自洽或自己喜欢的方式简单了解即可，能了解多少就算多少。}毕竟成分划分可是
语言专家的主战场呢。

本笔记采用夸克的分类方式，其实我对（\cref{tab:verbcop}）中很多非限定分句的句
法成分也不懂，哈哈。如英语功力深厚，英去夸克语法大全第16章一探究竟。
...
SVO中，\textbf{带主语的 -ing 从句} 可以作补足语。虽然\textbf{人称主语可用属格
  形式}，但常常使人感到别扭或不自然。
\begin{itemize}
...
\item[外置主语 (EXTRAPOSITION)] \index{概念!外置主语 extraposition} 句子
  ...
  \item Would \unbf{it} be better \unbf{to pay now}?
  \end{itemize}
\end{description}
...
\textbf{介词as（作为） 表示连系关系，特别是说明与直接宾语有关的角色或地位。}
\begin{itemize}
\item We considered him $ \left\{
  ...
  \right. $
\end{itemize}
但在某些方面，介词as和引导比较从句的连词as相似，\textbf{一方面引导从句；另一
  方面又引导和从句同位的名词短语}：
\begin{itemize}
...
\begin{table}[htbp]
  \centering
  \begin{talltblr}[ caption = {SVOC和SVOA 中的非限定性从句},
    label = {tab:svocin},
    ]{
...
要\textbf{注意歧义}，如left sb/sth to do sth和left sb/sth doing sth：
\begin{itemize}
\item Tom left her $\left\{
    \begin{aligned}
      &\text{to finish the job. [Tom离开她，去完成工作（目的）。]} \\
      &\text{finishing the job. [Tom离开她；她完成工作。her 上升宾语]} \\
    \end{aligned}
    \right.$
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{talltblr}[ caption = {宾语和介词宾语},
    label = {tab:PrepObj},
    note{a} = {$O_i$ indirect objects间接宾语，$O_d$ direct objects 直接宾
    语,$O_p$ preposition objects 介词宾语},
    ]{
...
\section{限制性关系从句作后置修饰语}

后置修饰的限制性从句可包括：
\begin{description}
\item[（形容词性）关系从句] 见\cref{chap:adjclause}，略。
...
\item[同位从句] 同位语从句（Appositive clause）是一种名词性从句，它在复合句
  ...
  idea, proposition, reply, remark, answer, news 等等。
  \begin{itemize}
...
其实我认为可以这样简单粗暴理解，\textbf{现在分词从句可以是关系从句中进行体的
  缩略形式，也可以是关系从句中（不带连系动词）一般式的缩略形式。}


\subsubsection{ -ed 分词从句作后置修饰语}

和 -ing 从句一样, -ed 分词从句仅与\textbf{关系代词作主语的关系从句}相对应：
\begin{itemize}
...
\textbf{由不定式从句作同位后置修饰语可能没有对应的同位限定性从句}，而只有一个
介词短语和它相对应：
\begin{itemize}
