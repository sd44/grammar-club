...
也就是：两个句子间要有一个\textbf{重复的元素}，由此建立“关系”，才可以用关系分句的方式
来合成复句。例如：
...
\item I know \unct{this}{O} \unct{because}{连接词} \unct{I have proof}{副词从句}.
  ...
  我知道，因为我有证据。
\item I don't trust \unct{people}{O} \unct{who talk too much}{关系分句}.
...
\item You can find \unct{\textnormal{two apples}}{先行词}
  \unct{\textnormal{which I bought}}{关系分句} in the basket. （误）
\end{itemize}
...
如果把 in the basket 向前移，把关系分句挪后，就成为：
\begin{itemize}
\item You can find \unct{\textnormal{two apples}}{先行词} in the basket
  \unct{\textnormal{which I bought}}{关系分句}. （误）
\end{itemize}
...
\item \unct{A plague}{\textnormal{先行词}} \unct{which lasted 20
    years}{\textnormal{关系分句}} broke out. （误）
\end{itemize}
...
\begin{enumerate}[resume]
\item \unct{A plague}{先行词} broke out \unct{which lasted 20 years}{关系分句}.
...
总之，关系分句与其他的修饰语相同，应该尽量靠近修饰的对象，这是为了表达清楚起
...
\begin{enumerate}[resume]
\item The rain came \unct{when the farmers needed it most}{关系分句}.
\end{enumerate}

关系分句
\begin{enumerate}[resume]
\item The rain came at a time \unct{the farmers needed it most}{关系分句}.
...
\begin{enumerate}[resume]
\item I know \unct{the time}{先行词} \unct{(when) he will arrive}{关系分句}.
\end{enumerate}
这个句子当然也可以省掉关系副词 when，成为：
\begin{enumerate}[resume]
\item I know \unct{the time}{先行词} \unct{he will arrive}{关系分句}.
\end{enumerate}
...
\begin{itemize}
\item I know \unct{the time}{O} \unct{he will arrive}{关系分句}.
\end{itemize}
...
\reitem\label{3road} (A) The car stopped (at \unct{a
  place}{先行词}) \unct{(where) three roads met}{关系分句}.
...
\item \unct{The Johnsons}{S} \unct{have}{V} \unct{a place}{先行词}
  \unct{(where) they can get away from other people.}{关系分句}
...
\item I called \unbf{for a reason}.
...
\item \unct{Whatever (= No matter what) he may say}{副词从句}, \unct{I}{S} \unct{won't change}{V} \unct{my mind}{O}.
  ...
  不管他会怎么说，我主意已定。
\item \unct{Whatever (= Anything that) he may say}{S 关系分句省略先行词成为名词
...
\item \unct{Whichever (=No matter which) way you go}{副词从句},
  ...
  不论你走哪条路，我都跟定你了。
\item \unct{Whichever way (=Any way that) you go}{S 关系分句省略先行词成为名词
...
形容词从句就是关系分句，主、从两个从句间一定有重复的元素以建立关系。既然有重
...
\reitem (A) The man \unct{whom you asked about}{关系分句} is here.
\reitem (B) The man \unct{you asked about}{关系分句} is here.
你要找的人在这儿。
\end{enumerate}
...
\item \unct{It}{S} is chilled to 6°C.
  ...
  \reitem (A) Beer \unct{which is chilled}{关系分句} to 6°C is most delicious.
...
关系分句简化后剩下的补语是过去分词短语，属于形容词类,而原来的从句也是形容词类，
所以没有词类的冲突，可以取代关系分句来形容beer，而且意思不变，这就是成功的简
化从句。再举一个有逗号的关系分句为例：
\begin{itemize}
\item \unct{Your brother John}{先行词}, \unct{who was wounded in war}{关系分句}, will soon be sent home.
...
与be 动词省略即可：
\begin{itemize}
\item \unct{Your brother John}{先行词}, \unct{wounded in war}{关系分句}, will soon be sent home.
\end{itemize}
...
例如：
\begin{itemize}
\item \unct{The ship}{先行词} \unct{which is coming to shore}{关系分句} is from Gaoxiong.
...
动词，只要省去这两个部分，就成为：
\begin{itemize}
\item The ship \unct{coming to shore}{关系分句} is from Gaoxiong.
\end{itemize}
...
\begin{itemize}
\item John is \unct{the one}{先行词} \unct{who should go this time}{关系分句}.
...
\begin{itemize}
\item   John is the one \unct{to go this time}{简化关系分句}.
\end{itemize}
...
\begin{itemize}
\item John is not \unct{a man}{先行词} \unct{whom one can trust}{关系分句}.
\end{itemize}

因为关系分句中的关系词 whom 是宾语，可以省略，成为：
\begin{itemize}
\item John is not \unct{a man}{先行词} \unct{one can trust}{关系分句}.
\end{itemize}

这个关系分句中的主语是空洞的 one，可以简化，再把 can trust 简化为 to trust，
...
就会成为这个复句：
\begin{itemize}
\item John is not \unct{a man}{先行词} \unct{who can be trusted}{关系分句}.
\end{itemize}
...
为：
\begin{itemize}
\item This is exactly \unct{the thing}{先行词} \unct{we should do}{关系分句}.
\end{itemize}

再把关系分句中的主语 we省略（因为对方知道你在说谁），把助动词改为不定式，就得
...
\begin{itemize}
\item This is exactly \unct{the time}{先行词} \unct{when we should do}{关系分句}.
\end{itemize}

关系副词非主要词类，在前面没有逗号的情况下可以径行省略，成为：
\begin{itemize}
\item This is exactly \unct{the time}{先行词} \unct{we should do it}{关系分句}.
\end{itemize}
...
如果对方知道你的意思，那么关系分句的主语 you 就可省略，再把 must 简化为
to，即成为：
\begin{itemize}
...
用的介词是for。例如：
\begin{itemize}
\item I have \unct{a job}{先行词} \unct{that your brother can do}{关系分句}.

  我有件差事想请你哥哥来做。
\end{itemize}
关系分句的关系词 that 是宾语，可以径行省略，成为：
\begin{itemize}
\item I have \unct{a job}{先行词} \unct{your brother can do}{关系分句}.
\end{itemize}
...
词类的冲突。例如：
\begin{itemize}
\item \unct{Bill Clinton}{先行词}, \unct{who is President of the U.S.}{关系分句},
  is a Baby Boomer.
...
留下来。再看这个例子：
\begin{itemize}
\item \unct{Although he opposed it}{副词从句}, the plan was carried out.
...
\item The President is a man on \unnormal{whom}{关系词} \unnormal{a heavy
    responsibility}{关系分句主语}, whether he likes it or not, falls. （不
  佳）
\end{itemize}
