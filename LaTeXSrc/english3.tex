...
\item[主语和动词的倒装] SVC 和 SVA 类型的分句大部分有必不可缺的第三成分，因为
  ...
  \item[AVS] Here \unbf{lies} \unbf{the Squidward’s hopes and dreams}. [尾重，
    在lies这样沉重悲伤的词之后\textbf{突出主语}，渲染到位。]
...
\item[主语和功能词的倒装]
  ...
  \item[so, neither, nor]
    \begin{itemize}
    \item John saw the accident and \unbf{so} did \unbf{Mary}. [Mary did so]
    \item She wasn't angry and \unbf{neither} was \unbf{I}. [I wasn't, either]
    \item She must come and \unbf{so} must \unbf{you}. [you must come too]

    \item 如果想将重心放在动词上，也可只前置 so.

      You asked me to leave, \unbf{so} \unbf{I} did.
    \end{itemize}
  ...
  \item[主语不是人称代词的比较分句]
    ...
    \item \unbf{I} spend more than do \unbf{my friends}.

    \item \unbf{Oil} costs less than would \unbf{atomic energy}.

      \unbf{Oil} costs less than did \sout{it}. [人称代词作主语的 than分句不能倒装]

    \item She is a doctor, \unbf{as} is \unbf{he}. [人称代词作主语的 than分句
      可以倒装]
...
\index{概念!分裂句 cleft sentence} 分裂句是是将一个句子分裂成两个分句，其中一
...
\item[it 子句] It + BE + \textbf{focus} + (that, who, whom, which, 零)关系分句，
  如：
  \begin{itemize}
  \item It is \unbf{Simon} \ul{who’s gone down}.

  \item It was \unbf{here} \ul{that the young girl first fell in love}.
...
\item[wh- 分句或假拟分裂句] Wh+ 主语 + 谓语 + 其他成分 + BE + \textbf{focus}
  ...
  \begin{description}
  \item[名词短语] \ul{What I saw} was \unbf{one of the most impressive government policies in years}.

    我所看到的是近年来最令人印象深刻的政府政策之一。
  \item[动词短语] \ul{What you do} is \unbf{wear it like that}.

    你要做的是像那样穿衣。

   \ul{What I did} was \unbf{send a complaint to Radio 2}.
