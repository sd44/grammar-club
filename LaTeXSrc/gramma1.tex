...
但以下情况不加the：
\begin{description}
\item[不与其他实体进行比较] 仅限于对某一实体的修饰，不特定。
  \begin{itemize}
  ...
  \item Mangoes are \unbf{(the) most} expensive in early summer.
    ...
    上句如加the其实是\textbf{非正式用法}。它没有涉及与其他实体的比较；并
    且most在这里其实是\textbf{表语}，不是名词短语中的\textbf{定语}（见
    \cref{sub:attrpred}）。
  \end{itemize}
...
\item[命令性 (MANDATIVE)] 分句谓语使用\textbf{动词原形}，用在that分句中。that分句由一
  ...
  \begin{itemize}
  \item It's essential that this mission \unbf{not fail}. [否定形式不加助动词do，虚拟语
    气]
...
PAST) 和假设过去完成体 (HYPOTHETICAL PAST PERFECTIVE)。
