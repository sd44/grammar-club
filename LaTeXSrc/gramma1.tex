...
了一个更为宽泛的补语超集：补足语 (COMPLEMENTATION)\index{概念!补足语 complementation}。
\begin{description}
...
\item[递差 (GRADIENCE)] 英语中常常有两个或多个类别之间相近，但有着不同程度级
  别的差异（级差）。如\ccref{tab:auxverb}，\ccref{tab:comparison}等。
\end{description}
...
\item This is \unbf{the most} expensive book in my collection.

\item Anna is \unbf{their youngest} child.
\end{itemize}
...
\item[已有冠词、所有格、人名或地理名称] 当名词短语中已有冠词、所有格代词、人
  ...
  \item The Romans conquered \unbf{most of England.}
  \end{itemize}
...
\item (C) \textbf{名词短语}冠词（a）与名词（road）之间是\textbf{前置修饰语位
    置}，而且\textbf{只能放一个“重量”过重的单词，不能放短语}（详
...
现代英语语法一般认为，英语没有将来时，因为它没有像许多其他语言那样的将来时变
...
\begin{figure}[p]
  ...
  \caption{\label{fig:alltense}英语全时态12种}
\end{figure}
...
动词原形。我觉得，最好还是将动词原形与不带to的不定式区分开为好，因为不带to不
定式之前有“体”的变化，以及“不定式”可以表示词性多变。
...
\item[不带to的不定式] 用在\textbf{否定助动词}，\textbf{情态助动词}（can, may, could等
...
不同于原形，两种不定式都\textbf{没有时态的标记，但有体的变化。} 如should/to \unbf{write},
should/to \unbf{be} writing, should/to \unbf{have} written, should/to \unbf{have} been writing,
didn't \unbf{do}.
...
\begin{itemize}
\item  \unct{That anybody picks strawberries}{S} \unct{can be}{V} \unct{fun}{C}.
\end{itemize}
...
这里的副词 especially 其实不是直接修饰一个名词spinach ，而是修饰``spinach
are good for you''这一隐含从句。
...
\item If an asteroid \unbf{should hit} the earth, man \unbf{could die} out.
  ...
  \textbf{假设从句和母句均使用有过去形态、但不代表过去式的助动词来表示发生可能性很低，非事实。}
...
\item If I \unbf{had studied} harder \unbf{in school}, I \unbf{could qualify} for the job
  ...
  \textbf{假设从句是过去时间（在学校时）的假设，母句是现在时间（now）。}
\end{itemize}

\textbf{也可能混合假设与事实：假设从句为假设，用假设条件语法；母句为事实，用普通语法。}例
如
...
\index{概念!同位语}等看过后面对“简化从句”的探讨，会了解到\textbf{所谓同位语
  其实就是形容词从句简化之后所留下的补语。关系从句是形容词类，如果简化主语
  和 be 动词，剩下的是名词补语，其词类虽与原来的关系从句词类有冲突，但仍然可
