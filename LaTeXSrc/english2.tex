...
\subsection{限定性分句省略方法}


由于\textbf{非限定性动词分句}（见 \cref{subsec:iffinite}）没有时态标记和情态
助动词,又常常没有主语和从属连词；也可以\textbf{根据句子的语境来还原时态、语态、
  人称和数}，因此是一种很有价值的\textbf{压缩句子}的方法。

\begin{itemize}
\item \unbf{When} (she was) \unbf{questioned}, she denied being a member of the group.
\item (Since/Because/As they were) considered works of art, they were
...
\textbf{说话者}。
\begin{itemize}
\item To be an administrator is to have the worst job in the world. [For
  a person to be \ldots{}]
\item It's hard work to be a student. [不确定主语， 如: anyone]
\item It's hard work, to be honest. [I 作主语]
\end{itemize}

助动词 have 有时用在 to 不定式中 (to have happened) 和-ing分词
中(having happened)，前者更能表示未来时间或不确定性。

\subsection{无动词分句省略方法}

无动词分句通常可以被认为是\textbf{省去动词 be} 的形式。
\begin{itemize}
...
\item One should avoid taking a trip abroad in August where (\unbf{it
    is}) possible.
\end{itemize}
