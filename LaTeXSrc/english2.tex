...
置结构看成是缩简的关系从句（去掉了重复主语代词连接词和be动词的关系从句）：
\begin{itemize}
...
where, when和用的较少的why还可以\textbf{引导关系从句（形容词从句）}：
\begin{itemize}
\item the place
    $\left\{
      \begin{aligned}
...
\textbf{尽管that从句和不定式从句可以起名词作用，但是它们不能作介词补语。}\textbf{人
称主格} I, you等也不能。
...
有些 -ing 和 -ed 从句形式就可以有边缘介词 (marginal preposition)、非限定动词
...
\textbf{复合句}像简单句,因为它\textbf{只包含一个主要从句}；但是和简单句不一样
的是,它\textbf{有一个或一个以上的从句作为它的句子成分。}
...
夸克使用\textbf{母句} (MATRIX clause)来称呼去除了从句的母句。
...
如果整个时间段都在过去，那么母句和从句都使用\textbf{过去完成时或一般过去时}：
\begin{itemize}
...
\item[关系从句 RELATIVE CLAUSES] \index{概念!关系从句 relative clauses} 功能上与修饰性形容词相同，修饰名词短语；位置上和后置修
  饰的介词相同。
  \begin{itemize}
...
\subsubsection{wh- 名词性关系从句}
\label{subsubsec:whnoun}

\textbf{名词性关系从句}也叫\textbf{自由关系从句}或者\textbf{独立关系从句}。
...
\item \unbf{My forgetting her name} was embarrassing.
  ...
  \unbf{I, who forgot her name,} was embarrassed. [名词性 -ing 从句可用来指一
